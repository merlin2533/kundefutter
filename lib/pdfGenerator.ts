...
/**
 * Erzeugt die Sammelrechnung-PDF für eine BEREITS bestehende Sammelrechnung (rein lesend,
 * legt nichts an) — einzige Quelle der Wahrheit für dieses Dokument, genutzt sowohl von
 * `GET /api/exporte/sammelrechnung` (erneutes Ansehen/Herunterladen) als auch von
 * `POST /api/exporte/sammelrechnung` (Sofort-Download direkt nach dem Anlegen aus
 * LieferhistorieTab). Rechnet je Position mit dem eingefrorenen `Lieferposition.mwstSatz`
 * (Fallback `Artikel.mwstSatz`) und Rabatt — nicht mehr mit einem pauschalen 19%-Satz ohne
 * Rabattberücksichtigung wie in einer früheren Version dieser Funktion, die dadurch bei
 * gemischten MwSt-Sätzen oder Rabatten einen falschen Gesamtbetrag auswies.
 */
export async function generiereSammelrechnungPdf(sammelrechnungId: number): Promise<Buffer> {
  ...
  if (!sammelrechnung) throw new Error(`Sammelrechnung ${sammelrechnungId} nicht gefunden`);
  ...
  const mwstSatzVon = (p: { mwstSatz?: number | null; artikel: { mwstSatz: number | null } }) =>
    p.mwstSatz ?? p.artikel.mwstSatz ?? 19;
  const nettoVon = (p: { menge: number; verkaufspreis: number; rabattProzent?: number | null }) =>
  ...
  // ── Summen ────────────────────────────────────────────────────────────────
  // Je tatsächlichem MwSt-Satz gruppiert (eine Sammelrechnung kann Positionen mit
  // unterschiedlichen Sätzen enthalten, z.B. Futter 19% + Dünger 19% + ein 7%-Artikel) —
  // nicht mehr ein pauschaler Satz für die gesamte Rechnung.
  const mwstGruppen = new Map<number, number>();
  let nettoGesamt = 0;
  for (const p of allPositionen) {
    const netto = nettoVon(p);
    nettoGesamt += netto;
    const satz = mwstSatzVon(p);
    mwstGruppen.set(satz, (mwstGruppen.get(satz) ?? 0) + netto);
  }
  let mwstGesamt = 0;
  ...
  for (const [satz, basis] of Array.from(mwstGruppen.entries()).sort(([a], [b]) => a - b)) {
  ...
  doc.setLineWidth(0.4);
  doc.line(sumX, sumY + 1, 196, sumY + 1);
  sumY += 8;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(11);
  ...
  doc.setFontSize(9);
  doc.setTextColor(60);
  doc.text(`Zahlbar innerhalb von ${sammelrechnung.zahlungsziel} Tagen nach Rechnungserhalt.`, 14, sumY + 12);
  if (FIRMA.iban) {
    doc.text(`Bankverbindung: ${FIRMA.bank}  ·  IBAN: ${FIRMA.iban}  ·  BIC: ${FIRMA.bic}`, 14, sumY + 18);
  }
  if (FIRMA.steuernummer) {
    doc.text(`Steuernummer: ${FIRMA.steuernummer}`, 14, sumY + 24);
  }
