...
/api/lieferungen/[id]/restdifferenz      POST({gutschriftId?}) — verrechnet den nach Teilzahlungen noch offenen Betrag
                                 einer bereits gestellten Rechnung (z.B. Kunde hat eine Gutschrift falsch/zu viel
                                 abgezogen und zu wenig überwiesen): optional eine OFFENE Gutschrift des Kunden
                                 gegenbuchen (VERBUCHT, ohne neue Position — Rechnung steht bereits), ein danach
                                 verbleibender Restbetrag wird als KundeForderung angelegt und automatisch in die
                                 nächste Rechnung dieses Kunden übernommen (siehe verrechneOffeneRestdifferenz() in
                                 lib/lieferung.ts); 400 falls die gewählte Gutschrift den offenen Betrag übersteigt
