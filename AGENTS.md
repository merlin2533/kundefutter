...
identisch** (aktuell 45.668 Zeichen) — inklusive des Produktlinien-Umschalters
...
├── img/
│   ├── favicon.svg
│   ├── og-preview.png · og-preview-eierhandel.png  ← Social-Vorschau 1200×630, je Produktlinie
│   ├── hero_bg.png · crm_sales.png · warehouse.png
│   ├── truck_delivery.png · smartphone_scan.png · commodities.png
│   └── *.webp          ← WebP-Variante jedes Fotos (Pflicht, siehe Bild-Regel unten)
└── video/
...
- **Neues Foto → immer zusätzlich als WebP ablegen und über `<picture>` einbinden**, PNG bleibt
  als `<img>`-Fallback stehen:
  ```html
  <picture>
    <source srcset="img/name.webp" type="image/webp" />
    <img src="img/name.png" alt="…" class="mockup-bg-img" loading="lazy" />
  </picture>
  ```
  Beim Hero-Hintergrund läuft dasselbe über zwei CSS-Zeilen: erst die PNG-`background`-Kurzform,
  danach `background-image: image-set(…)` — Browser ohne `image-set()` behalten so das PNG. Die
  sechs Fotos wiegen als PNG zusammen 5.071 KB, als WebP 742 KB (−85 %); der Hero wird zusätzlich
  per `<link rel="preload" as="image" type="image/webp">` vorgeladen. `.mockup-wrapper picture`
  hat `display: contents`, damit das `<picture>` keinen eigenen Kasten ins Layout einzieht
  (das `<img>` darin ist `position:absolute`)
- **`og:image` je Produktlinie**: `index.html` → `img/og-preview.png`, `eierhandel.html` →
  `img/og-preview-eierhandel.png` (je 1200×630). Erzeugt werden sie aus dem Design-System selbst
  (HTML-Karte + Playwright-Screenshot), nicht von Hand gemalt — nach einer Änderung an Claim oder
  Farbwelt neu rendern und in `sitemap.xml` als `<image:image>` mitführen
...
Benutzer            — Multi-User (benutzername, passwortHash, rolle, aktiv, mobil? — erscheint als
                      ...
                      mitarbeiterId? — weiche ID ohne @relation analog Kontoumsatz.gutschriftId auf
                      genau einen Mitarbeiter-Stammdatensatz; macht diesen Login unabhängig von
                      rolle/rolleId/berechtigungen zu einem Personal-Selbstbedienungs-Account, der
                      nur /meine-arbeitszeit sieht, siehe istPersonalSelbstbedienung() in
                      lib/permissions.ts)
...
│   ├── neu/page.tsx            Einzeln- + Batch-Modus (?modus=batch, Serienfoto → Übersicht offener Batches)
│   ├── [id]/page.tsx
│   └── batch/[id]/page.tsx     Batch-Review: je Beleg KI-Ergebnis prüfen/korrigieren, einzeln oder gesammelt übernehmen
├── bankabgleich/
...
├── meine-arbeitszeit/page.tsx  Personal-Selbstbedienung: eigene Arbeitszeit erfassen/einsehen
│                               (für Benutzer.mitarbeiterId-verknüpfte Logins; Nav zeigt nur diesen
│                               Eintrag + meine-lohnabrechnungen, jede andere Seite redirected hierher)
├── meine-lohnabrechnungen/page.tsx  Personal-Selbstbedienung: eigene Gehaltsabrechnungen einsehen
│                               (nur ABGERECHNET/AUSGEZAHLT, kein OFFEN-Entwurf); Link öffnet das vom
│                               Steuerberater hochgeladene Original-PDF (GET .../[id]/beleg), NICHT
│                               die intern generierte Druckseite — fehlt das PDF, "Noch nicht hinterlegt"
...
/api/exporte/auftragsbestaetigung       GET?lieferungId= — Auftragsbestätigung-PDF (nur sinnvoll solange status "geplant"; nur Positionen/Menge/Einheit, OHNE Preise/MwSt, OHNE Signatur/Chargen)
...
- Auftragsbestätigung: `/lieferungen/[id]/auftragsbestaetigung/page.tsx` (nur solange status "geplant"; NUR Positionen/Menge/Einheit ohne Preise/MwSt — siehe Bug-/Feature-Tabelle unten, `GET`/`POST .../mail` analog Lieferschein, `generiereAuftragsbestaetigungPdf()`)
...
| Mahnwesen (`/mahnwesen`): eine dem Kunden bekannte, tatsächlich noch offene/überfällige Rechnung ließ sich über das Suchfeld nicht finden ("0 Einträge angezeigt"), obwohl sie eigentlich in der Liste stehen müsste | Derselbe "500er-Deckel"-Bug-Typ wie schon mehrfach dokumentiert (`GET /api/kunden`, `GET /api/lieferungen`, `GET /api/kunden/[id]`), diesmal in `GET /api/mahnwesen`: die zugrunde liegende Query lädt ALLE gelieferten, unbezahlten Lieferungen mit Rechnungsnummer (nicht nur die bereits überfälligen — die Überfälligkeits-Prüfung passiert erst danach in JS) mit `orderBy:{datum:"asc"}, take:500` — bei mehr als 500 insgesamt offenen Rechnungen fallen die NEUESTEN heraus (älteste 500 gewinnen), unabhängig davon, ob sie selbst schon überfällig sind. Das clientseitige Suchfeld (`app/mahnwesen/page.tsx`) filtert korrekt per Teilstring auf Kunde/Firma/Rechnungsnr., kann aber naturgemäß nur innerhalb der bereits vom Server gelieferten (und damit schon gekürzten) Liste suchen — der Fehler liegt vollständig serverseitig, nicht im Suchfeld selbst | `take: 500` in `app/api/mahnwesen/route.ts` auf `take: 5000` angehoben (gleiches Muster/gleiche Obergrenze wie bei den anderen bereits gefixten Stellen). End-to-end gegen die echte DB verifiziert: Testkunde mit 505 älteren offenen Füll-Rechnungen + 1 neuerer, ebenfalls überfälliger Ziel-Rechnung (Nummer mit "9375") angelegt — mit `take:500` (altes Verhalten) fehlt die Ziel-Rechnung in der API-Antwort (500 von 506 Einträgen), mit `take:5000` (Fix) erscheinen alle 506 korrekt inkl. der Ziel-Rechnung |
| Arbeitszeiterfassung (`/personal/arbeitszeiterfassung` UND `/personal/[id]/stunden/neu`): ein Mitarbeiter mit NUR EINEM konfigurierten Wochentag in `bevorzugteArbeitszeiten` (z.B. nur Donnerstag, echter Fall: Mitarbeiter, der ausschließlich donnerstags arbeitet) bekam an jedem ANDEREN Wochentag (z.B. Montag) trotzdem automatisch 8 Stunden als Vorschlag vorbelegt, statt 0 — obwohl für diesen Tag explizit gar keine Arbeitszeit hinterlegt war | `sollStundenFuerDatum()` (`lib/utils.ts`) prüfte nur, ob der konkrete Wochentag-Key im `bevorzugteArbeitszeiten`-JSON einen Zahlenwert hat; fehlte er (weil der Mitarbeiter bewusst nur einen Teil der Woche konfiguriert hat), fiel die Funktion in den allgemeinen Fallback `wochenstunden/5` bzw. — ohne gepflegtes `wochenstunden` — den hart codierten Standardwert 8h, GENAU SO als hätte der Mitarbeiter überhaupt keine bevorzugten Arbeitszeiten gepflegt. Der Fallback ist aber nur für "keine Planung hinterlegt" gedacht, nicht für "dieser einzelne Tag fehlt in einer ansonsten bewusst unvollständigen Planung" | `sollStundenFuerDatum()` unterscheidet jetzt zwischen beiden Fällen: Ist überhaupt ein `bevorzugteArbeitszeiten`-Objekt vorhanden (auch mit nur einem einzigen konfigurierten Tag), gilt das als vollständige, bewusste Wochenplanung — ein darin nicht aufgeführter Tag liefert jetzt 0 ("arbeitet an diesem Tag nicht"), nicht mehr die Standardschätzung. Der `wochenstunden/5`- bzw. 8h-Fallback greift nur noch, wenn `bevorzugteArbeitszeiten` komplett `null`/nicht gepflegt ist. Neue Unit-Tests in `__tests__/lib/utils.test.ts` (nicht aufgeführter Tag → 0 trotz gesetztem `wochenstunden`, konfigurierter Tag → korrekter Wert, kompletter Fallback ohne jede Konfiguration bleibt unverändert). End-to-end gegen die echte DB + Playwright verifiziert: Testmitarbeiter mit `bevorzugteArbeitszeiten:{"do":8}` (kein `wochenstunden`) zeigt auf `/personal/arbeitszeiterfassung` UND `/personal/[id]/stunden/neu` für Montag jetzt korrekt 0 (vorher 8), für Donnerstag weiterhin korrekt 8, für Samstag weiterhin 0 |
| Arbeitszeiterfassung erfasste nur eine reine Gesamtstundenzahl je Tag, keine tatsächliche Kommen-/Gehen-Uhrzeit — arbeitsrechtlich unzureichend für die Aufzeichnungspflicht (Beginn/Ende der täglichen Arbeitszeit, nicht nur die Dauer). Auch die "bevorzugten Arbeitszeiten" je Wochentag (Vorbelegung) kannten nur eine reine Stundenzahl, keine Uhrzeit | `Arbeitsstunde` hatte kein Feld für Kommen-/Gehen-Zeit, nur `stunden` (Float). `Mitarbeiter.bevorzugteArbeitszeiten` speicherte je Wochentag ausschließlich eine Zahl (`{mo:8,...}`), kein Zeitfenster | Neue Felder `Arbeitsstunde.von`/`bis String?` (Migration `20260911213000_arbeitsstunde_zeiten`, Format "HH:MM") für die tatsächliche Uhrzeit — optional, da Urlaub/Krank/Feiertag und Alteinträge keine Uhrzeit kennen. `Mitarbeiter.bevorzugteArbeitszeiten` unterstützt je Wochentag jetzt zusätzlich zur bisherigen reinen Stundenzahl (Legacy) ein Zeitfenster-Objekt `{von,bis}` — beide Formen bleiben nebeneinander gültig, keine Zwangsmigration von Bestandsdaten. Neue zentrale Helfer in `lib/utils.ts`: `parseUhrzeit()`/`stundenZwischenUhrzeiten()` (HH:MM → Dezimalstunden), `istBevorzugtesZeitfenster()`/`stundenAusBevorzugtemTag()` (Legacy-Zahl vs. Zeitfenster unterscheiden), `bevorzugtesZeitfensterFuerDatum()` (Zeitfenster-Vorschlag für einen Wochentag, `null` bei Legacy-Zahl oder nicht konfiguriertem Tag) — `sollStundenFuerDatum()` nutzt dieselbe Unterscheidung intern mit. `POST`/`PUT /api/personal/arbeitsstunden(/[id])` berechnen `stunden` automatisch aus `von`/`bis`, sobald beide gültig gesetzt sind (Vorrang vor einer manuell übergebenen Stundenzahl — die Uhrzeit ist der maßgebliche Wert), 400 bei ungültigem Zeitfenster (Bis muss nach Von liegen). Frontend: `/personal/[id]/stunden/neu` und die Sammelerfassung `/personal/arbeitszeiterfassung` zeigen bei Art "Arbeit" zusätzliche Von/Bis-Zeitfelder (vorbelegt aus `bevorzugtesZeitfensterFuerDatum()`, überschreibbar), das Stunden-Feld wird bei gesetztem Zeitfenster automatisch berechnet und schreibgeschützt (grau); die Stunden-Tabelle auf `/personal/[id]?tab=stunden` zeigt eine neue "Zeit"-Spalte mit dem Von–Bis-Bereich. Die Stammdaten-Bearbeiten-Karte "Bevorzugte Arbeitszeiten" bietet je Wochentag jetzt Von/Bis-Zeitfelder statt eines einzelnen Stundenwerts; ein Tag mit vorhandenem Legacy-Wert (reine Zahl ohne Uhrzeit) zeigt einen Hinweis ("bisher Xh ohne Uhrzeit hinterlegt — bleibt beim Speichern unverändert") und bleibt beim Speichern unangetastet, solange der Nutzer die Von/Bis-Felder für diesen Tag nicht selbst befüllt (verhindert stillschweigenden Datenverlust bestehender Konfigurationen); die Lese-Ansicht zeigt je Tag automatisch "Xh" (Legacy) oder "HH:MM–HH:MM" (Zeitfenster). 12 neue Unit-Tests in `__tests__/lib/utils.test.ts`. End-to-end gegen die echte DB + Playwright verifiziert: Mitarbeiter mit gemischter Konfiguration (Montag Legacy 8h, Donnerstag Zeitfenster 08:00–16:30) — Bearbeiten-Formular zeigt Montag korrekt leer mit Legacy-Hinweis, Donnerstag korrekt vorbefüllt; Speichern mit nur geänderter Donnerstag-Bis-Zeit lässt Montags Legacy-Wert unangetastet; Stunden erfassen für Donnerstag übernimmt Von/Bis automatisch, Stunden-Feld ist berechnet+gesperrt (9,0h bei 08:00–17:00), POST persistiert `von`/`bis` korrekt in der DB; Sammelerfassung zeigt dasselbe Zeitfenster vorbefüllt, manuelles Ändern von "Von" berechnet die Stundenzahl live neu (09:00–17:00 → 8,0h); ein API-Aufruf mit Bis vor Von liefert korrekt 400 |
...
| Im gemeinsamen CSS-Bereich beider Landingpages standen seitenspezifische Navigations-Regeln — `eierhandel.html` schleppte vier Ausblend-Regeln für Sprungmarken mit, die es dort gar nicht gibt (`#integrationen`, `#tourenplanung`, `#vergleich`, `#ki`) | Beim Anlegen von `eierhandel.html` wurde der `<style>`-Block von `index.html` wörtlich übernommen — inklusive der Regeln, die auf die Menüpunkte der Agrarhandel-Seite zielen. Die passenden Ersatzregeln kamen 500 Zeilen später dazu, die toten blieben stehen. Damit stimmte auch die in AGENTS.md dokumentierte Zusicherung nicht mehr, der gemeinsame Bereich sei per `diff` vergleichbar (24 abweichende Zeilen) | Alles, was auf beiden Seiten gleich gilt (inkl. `.brand-switch` und der gemeinsamen Breakpoints), steht jetzt im geteilten Bereich; alles Seitenspezifische in einem eigens überschriebenen Block dahinter (`SEITENSPEZIFISCH — Agrarhandel` bzw. `EIERHANDEL-ERGÄNZUNGEN`). Der geteilte Bereich ist wieder Zeichen für Zeichen identisch, AGENTS.md enthält das Prüfskript dazu |
| Beide Landingpages luden 5.071 KB Fotos als PNG — auf dem Mobilfunknetz des Außendiensts (genau die Zielgruppe) der mit Abstand größte Posten der Seite, dreimal so viel wie alles andere zusammen | Die sechs Fotos lagen ausschließlich als PNG vor; PNG ist für Fotos das falsche Format (verlustfrei, keine Chroma-Unterabtastung). Beim Bau der Seite nie umgestellt | Jedes Foto zusätzlich als WebP (`hero_bg` q65, Rest q80) → zusammen 742 KB, **85 % weniger**, 4,2 MB gespart. Einbindung ohne Bruch für alte Browser: die fünf bzw. drei Mockup-Fotos über `<picture>` + `<source type="image/webp">` mit unverändertem `<img src="…png">` als Fallback, der Hero-Hintergrund über zwei CSS-Zeilen (erst `background: url(.png)`, dann `background-image: image-set(…)` — wer `image-set()` nicht kennt, behält die PNG-Zeile aus der Kaskade). `<link rel="preload" as="image" type="image/webp">` zieht das Hero-Bild vor. `.mockup-wrapper picture { display: contents; }` verhindert, dass das neue `<picture>` einen eigenen Kasten ins Layout einzieht — das `<img>` darin ist `position:absolute`, der Wrapper darf im Layout gar nicht erst auftauchen. Per Playwright verifiziert: `img.currentSrc` löst auf beiden Seiten und in beiden Viewports auf `.webp` auf, der Hero-Hintergrund ebenfalls, die Mockup-Höhen bleiben unverändert, kein horizontales Scrollen bei 390px |
| Beide Landingpages verwiesen per `og:image`/`twitter:image` (und `sitemap.xml`) auf `img/og-preview.png` — **die Datei existierte nicht**. Jeder Link, den jemand bei WhatsApp, LinkedIn oder Slack teilte, erschien dadurch ohne Vorschaubild; für eine Seite, deren einziger Zweck die Leadgewinnung ist, der teuerste denkbare stille Fehler | Das Meta-Tag war beim Bau der Seite gesetzt, das Bild nie erzeugt worden. Ein 404 auf `og:image` ist nirgends sichtbar — weder im Browser noch in der Konsole, nur beim tatsächlichen Teilen | Zwei Bilder (1200×630) erzeugt, je Produktlinie eines: `og-preview.png` (grün, Agrarhandel-Claim) und `og-preview-eierhandel.png` (Dotter-Akzent, Eierhandel-Claim) — `eierhandel.html` zeigte vorher auf die Agrarhandel-Vorschau, was die eigene Produktlinie beim Teilen unsichtbar gemacht hätte. Gerendert **aus dem Design-System selbst** (HTML-Karte mit denselben Farbtokens + Playwright-Screenshot), nicht von Hand gemalt — dadurch reproduzierbar, wenn sich Claim oder Farbwelt ändern. Beide auf 256 Farben quantisiert (271 KB statt 502 KB). `sitemap.xml` führt die Eierhandel-Vorschau jetzt als erstes `<image:image>` der Seite |
| `index.html` verwies im `<head>` per `<link rel="alternate" type="application/rss+xml">` auf `https://agri-office.de/feed.xml` — diese Datei existiert nicht und es gibt auch keinen Blog, nur einen Blog-**Teaser**-Abschnitt ohne Artikel dahinter | Beim Anlegen des Teaser-Abschnitts wurde der Feed-Link vorsorglich mitgesetzt, der Feed selbst nie gebaut | Link ersatzlos entfernt. Ein Feed-Discovery-Link auf eine 404 ist kein neutraler Platzhalter: Feed-Reader und Crawler folgen ihm und protokollieren den Fehler |
| Beide Landingpages beschrieben das Modul-System noch als „Branchen-Preset" — einen Knopfdruck, der mehrere Module auf einen Schlag setzt. Mit #433 ist daraus eine **Betriebsart** (`system.betriebsart`) geworden, die gesetzt bleibt und zusätzlich den Menüzuschnitt formt; die Einstellungsseite zeigt statt 16 Einzelschaltern sechs Bereiche. Die Seite versprach damit einerseits das falsche UI-Wort (ein Interessent findet im Demo-Termin kein „Preset"), andererseits war ihre Zusage „Navigation und Kundenakte zeigen dann nur noch, was Sie brauchen" vor #433 gar nicht eingelöst — ein Eierbetrieb sah weiterhin Zwischenfruchtrechner, Anbauplanung und Sachkundenachweise im Menü | Produkt und Marketing-Seite wurden in getrennten PRs geändert (#433 kam nach der Landingpage-PR #434 in `main`); nichts prüft, ob die Website noch beschreibt, was die Software tatsächlich tut | Feature-Karten, FAQ-Antworten und `llms.txt` auf beiden Seiten auf „Betriebsart" umgestellt, inklusive des jetzt tatsächlich zutreffenden Menü-Versprechens (Gruppe „Pflanze & Tier" → „Eier & Futter" beim Eierbetrieb) und der sechs Modul-Bereiche. `FAQPage`-Schema nicht von Hand nachgezogen, sondern über `scripts/sync-faq-schema.py` aus der sichtbaren FAQ neu erzeugt |
| Typografie-Fußangel im HTML: 14 Stellen schrieben `„Wort"` — deutsches öffnendes Anführungszeichen (U+201E), aber ASCII-`"` (U+0022) als Schlusszeichen. Genau das Muster, das schon einmal das komplette `FAQPage`-Markup auf `index.html` ungültig gemacht hat (siehe Zeile weiter oben) | Beim Tippen liegt das ASCII-`"` auf der Taste, das schließende `"` (U+201C) nicht — der Fehler entsteht automatisch, wenn man das öffnende Zeichen aus einer Vorlage kopiert und das schließende selbst tippt. Im JSON-LD war es diesmal nur deshalb harmlos, weil `sync-faq-schema.py` beim Erzeugen escapet (`\"`) — eine spätere Handkorrektur am Schema hätte den Block sofort gesprengt | Alle Schlusszeichen im **sichtbaren** HTML auf U+201C korrigiert (die JSON-LD-Blöcke bewusst ausgespart und stattdessen per `sync-faq-schema.py` neu erzeugt, damit die Escaping-Logik des Skripts die Quelle der Wahrheit bleibt). Prüfung als Einzeiler: `python3 -c "import io,re; [print(f, re.findall('„([^„“”\"<>]{1,90})\"', io.open(f,encoding='utf-8').read())) for f in ('web/index.html','web/eierhandel.html')]"` — muss leere Listen liefern |
| Die Mockup-Slideshow (Foto ↔ App-Ansicht, Umschalt-Dots unten in jedem Mockup) hängte ihre Dots an **jeden** `.mockup-wrapper` — auch an die beiden Deep-Dives „Compliance" und „Finanzen" auf `index.html`, die gar kein Hintergrundfoto haben. Ein Klick auf deren „Foto-Ansicht"-Dot blendete die App-Ansicht aus und zeigte eine leere dunkle Fläche | `initMockupSlideshow()` iterierte über `document.querySelectorAll('.mockup-wrapper')` ohne zu prüfen, ob der Wrapper überhaupt ein `.mockup-bg-img` enthält. Fiel beim Bau nicht auf, weil der Standardzustand die App-Ansicht ist — der kaputte Zustand entsteht erst durch einen Klick bzw. nach einem Timer-Durchlauf | Frühes `return` in der Schleife, wenn `wrap.querySelector('.mockup-bg-img')` leer ist — diese beiden Wrapper bekommen dadurch gar keine Dots mehr (vorher 2, jetzt 0; per Playwright über alle sieben Wrapper verifiziert). In beiden Dateien identisch, da das Skript geteilt ist |
| KI-Batch-Lieferschein-Erkennung (`/ki/lieferung/batch/[id]`): Nutzer meldete, die "Kunde"-Auswahl eines Belegs reagiere nicht — Klick auf einen Treffer im Dropdown ändere sichtbar nichts, der Beleg lasse sich dadurch nicht als "Passt" markieren. Gezielt nachgestellt (Testfixture mit exakt der gemeldeten Situation: KI erkennt keinen Kunden, zwei ähnlich benannte Kunden "Volker Vorhölter"/"Vorhölter" in der Auswahl) — der Klick selbst funktionierte im Test zuverlässig (Auswahl wird korrekt per PATCH persistiert), das eigentliche Risiko liegt aber im selben Bug-Mechanismus, der bereits `/lieferungen/neu` "springende"/scheinbar nicht reagierende Felder verursachte, hier aber nie mitgezogen wurde | `kundenOptions`/`artikelOptions` (`app/ki/lieferung/batch/[id]/page.tsx`) wurden per Inline-`.map()` bei JEDEM Render neu aus `kunden`/`artikel` (State, potenziell bis zu 5000 Einträge je Liste) berechnet, statt per `useMemo` — bei einem laufenden Batch mit mehreren Belegen löst nicht nur jeder Tastendruck in Menge/Preis/Chargennummer, sondern auch JEDER Analyse-Fortschritt eines ANDEREN, noch nicht fertigen Belegs (`verarbeiteAnalyseErgebnis()`s `setBatch()`-Aufruf) ein Re-Rendern der kompletten Seite und damit eine komplette Neuberechnung beider Listen aus — bei einem großen, echten Kunden-/Artikelbestand entsteht dadurch derselbe Main-Thread-Jank, der auf `/lieferungen/neu` bereits als Ursache für "Klick/Eingabe reagiert nicht" identifiziert wurde | `kundenOptions`/`artikelOptions` auf `useMemo(() => ..., [kunden])`/`useMemo(() => ..., [artikel])` umgestellt — identisches Muster wie der bereits bestehende Fix auf `/lieferungen/neu`. End-to-end mit Playwright gegen die echte DB verifiziert (vor UND nach dem Fix): Klick auf einen Kunden im Dropdown persistiert die Auswahl weiterhin korrekt per `PATCH .../items/[itemId]`, keine Regression durch die Memoisierung; `npx vitest run` weiterhin 248/248 grün |
| „Neue Sammelrechnung" (`/sammelrechnungen/neu`) zeigte für einen Kunden mit tatsächlich offenen, korrekt zugeordneten Lieferungen „Keine offenen Lieferungen ohne Rechnung gefunden." — obwohl weder eine Rechnungsnummer vorlag noch der Kundenname doppelt/anders geschrieben war (beides gezielt vom Nutzer ausgeschlossen). Ursache: der Betrieb markiert Lieferungen im Alltag nie explizit als „geliefert" (die Ware verlässt das Haus, ohne dass jemand den Status im System nachträgt) — genau dieser Workflow wird von einer Einzelrechnung längst unterstützt (`aktion=rechnung_erstellen` zieht den Übergang „geplant"→„geliefert" automatisch mit, siehe `markiereLieferungGeliefertFallsGeplant()`), von der Sammelrechnung aber nicht | Der Frontend-Fetch fragte hart `status=geliefert` ab (`app/sammelrechnungen/neu/page.tsx`), und `POST /api/sammelrechnungen` (`app/api/sammelrechnungen/route.ts`) bündelte Lieferungen nur per `updateMany({sammelrechnungId})`, ohne — wie die Einzelrechnung — je `markiereLieferungGeliefertFallsGeplant()` aufzurufen. Eine „geplante" Lieferung hatte dadurch zwar korrekt keine Rechnungsnummer, tauchte aber nie in der Auswahl auf, weil das gar nicht erst abgefragt wurde | Frontend fragt jetzt ohne `status`-Filter ab und filtert clientseitig zusätzlich `status !== "storniert"` (statt nur `!rechnungNr && !sammelrechnungId`); „geplante" Zeilen zeigen ein `StatusBadge` „Auftrag" plus Hinweistext, dass sie beim Erstellen automatisch auf „geliefert" gesetzt werden. `POST /api/sammelrechnungen` ruft dafür `markiereLieferungGeliefertFallsGeplant(tx, id)` für jede gewählte Lieferung **innerhalb derselben `$transaction`** auf (bucht Lagerausgang + Chargenpflicht-Prüfung nach, identisches Muster wie bei der Einzelrechnung) und lehnt zusätzlich `status:"storniert"`-Lieferungen serverseitig ab (bisher gar nicht geprüft — nur implizit durch den jetzt entfernten `status=geliefert`-Fetch-Filter der Wegbereiter). End-to-end gegen die echte DB verifiziert: zwei „geplante" Lieferungen + eine stornierte angelegt — alle drei DB-Zustände real gebucht (nicht nur simuliert); nach Erstellen der Sammelrechnung sind die zwei geplanten korrekt auf „geliefert" mit gebuchtem Lagerausgang (Bestand 500→486, `lagerBereitsGebucht:true`) und `sammelrechnungId` gesetzt, die stornierte bleibt unangetastet; ein direkter API-Aufruf, der die stornierte Lieferung mit hineinschummelt, wird mit 400 abgelehnt; `npx vitest run` weiterhin 272/272 grün |
| Gehaltsabrechnung (Tab "Abrechnung" auf `/personal/[id]`) auf dem Handy: der bereits vorhandene "Bearbeiten"-Button einer OFFEN-Abrechnung war unsichtbar/unerreichbar — der Screenshot des Nutzers zeigte die Zeile scheinbar direkt nach dem Status-Badge abgeschnitten, ganz ohne "Aktionen"-Spalte, obwohl das Feature laut Code bereits existierte (siehe Zeile weiter oben zu "Gehaltsabrechnung... ließ sich nicht mehr korrigieren") | `<table>` (Spalten Zeitraum/Brutto/Netto/Status/Aktionen, bei `stundenbasis`-Mitarbeitern zusätzlich Stunden) war direkt in eine Karte mit `overflow-hidden` eingebettet, ohne eigenen `overflow-x-auto`-Wrapper — bei 390px reicht der Platz für die Aktionen-Spalte (Bearbeiten/Abrechnen/Druck/Löschen) nicht, `overflow-hidden` schnitt sie einfach unsichtbar ab statt sie scrollbar zu machen. Identischer Bug-Mechanismus wie bereits bei `/gutschriften/[id]` behoben (siehe Zeile weiter oben), hier aber nie mitgezogen — zusätzlich unbehoben in den zwei Nachbar-Tabs "Stunden" (Datum/Art/Stunden/Löschen) und "Urlaub" (Von/Bis/Tage/Status/Aktionen) derselben Seite | Alle drei Tabellen auf `app/personal/[id]/page.tsx` (Abrechnung/Stunden/Urlaub) in einen `<div className="overflow-x-auto">` innerhalb der Karte gewrappt (äußeres `overflow-hidden` bleibt für die abgerundeten Ecken erhalten) — identisches, bereits etabliertes Muster wie beim Gutschrift-Fix. End-to-end gegen die echte DB + Playwright mit mobilem Viewport (390px) verifiziert: "Bearbeiten" ist jetzt per Wisch-nach-links innerhalb der Tabelle erreichbar (kein Overflow der gesamten Seite), Klick öffnet den Bearbeiten-Modus, Ändern von Brutto/Netto (603,00 € → 301,50 €) speichert korrekt und übersteht einen Reload; Stunden- und Urlaub-Tab zeigen weiterhin keinen Seiten-Overflow (keine Regression) |
| KI-Belegerkennung (`POST /api/ki/beleg`, genutzt von `/ausgaben/neu` + `/ausgaben/[id]`): der erkannte Netto-Betrag stimmte "häufig" nicht mit dem tatsächlich gedruckten Bon überein, immer nur um Cent-Beträge daneben (Beispielfall: Waschpark-Quittung, Brutto 13,01 € brutto bei 19% MwSt) | `PROMPTS.beleg` (`lib/ai.ts`) wies die KI an, den Netto-Betrag bei einem reinen Kassenbon (der i.d.R. nur den Bruttobetrag ausweist, kein separates Netto/MwSt) selbst "aus Brutto und MwSt zu berechnen" — eine Dezimal-Division auf den Cent genau ist aber keine verlässliche Freitext-Arithmetik für ein Sprachmodell und wich dadurch unsystematisch von der korrekten kaufmännischen Rundung ab; `app/api/ki/beleg/route.ts` übernahm den von der KI selbst gerechneten Wert ungeprüft | Prompt geändert: die KI liest jetzt NUR noch, was tatsächlich als Zahl auf dem Beleg steht (`betragNetto` bleibt `null`, wenn kein Netto-Betrag gedruckt ist — insbesondere bei einem einfachen Kassenbon der Regelfall), rechnet selbst nichts mehr um. Neue, deterministische `berechneNettoAusBrutto(brutto, mwstSatzProzent)` (`lib/utils.ts`, Brutto-first analog einer Kleinbetragsrechnung nach §33 UStDV: `rundeKaufmaennisch(brutto / (1 + satz/100))`) leitet das Netto stattdessen in `app/api/ki/beleg/route.ts` per Code ab, sobald die KI nur den Bruttobetrag liefert. 3 neue Unit-Tests in `__tests__/lib/utils.test.ts` (u.a. der reale 13,01-€-Fall aus der Meldung sowie ein Fall, bei dem die Division nicht glatt aufgeht, um die korrekte Rundungsrichtung abzusichern); `npx vitest run` weiterhin grün (275/275) |
| Ausgabenbuch (`/ausgaben`): der "Von"-Datumsfilter sprang beim erneuten Öffnen der Seite immer auf den 31. des VORMONATS zurück statt auf den 1. des aktuellen Monats — reproduzierbar Anfang jedes Monats während der Sommerzeit (z.B. 31.08. statt 01.09.) | `firstOfMonth`/`todayStr` in `app/ausgaben/page.tsx` bauten das Default-Datum über `new Date(jahr, monat, 1).toISOString().slice(0,10)` — `new Date(...)` erzeugt Mitternacht in der LOKALEN Zeitzone des Browsers, `toISOString()` rechnet das aber zuerst auf UTC um; in der deutschen Sommerzeit (UTC+2) landet Mitternacht des 1. dadurch auf 22:00 UTC des LETZTEN Tages des Vormonats, `.slice(0,10)` liest also fälschlich dessen Datum. Klassischer "lokales Datum via toISOString()"-Fallstrick — betrifft potenziell weitere der ca. 38 Dateien mit demselben Muster, hier aber bewusst nur die gemeldete Stelle gefixt | `firstOfMonth`/`todayStr` bauen das Datum jetzt direkt aus `getFullYear()`/`getMonth()`/`getDate()` zusammen (`${jahr}-${pad(monat+1)}-01` bzw. `-${pad(tag)}`), ganz ohne den UTC-Umweg über `toISOString()`. Per `node`-Testskript reproduziert (alte Formel liefert am 14.9.2026 in Europe/Berlin "2026-08-31", neue "2026-09-01") und end-to-end mit Playwright gegen die echte laufende App verifiziert (Browser-Kontext `timezoneId: "Europe/Berlin"`, eingeloggt, `/ausgaben` geöffnet): "Von"-Feld zeigt jetzt korrekt `2026-09-01` |
| Ausgabenbuch-Erfassung konnte pro Beleg nur EINEN MwSt-Satz erfassen — ein Bewirtungsbeleg mit Speisen (7 %) UND Getränken (19 %) auf derselben Rechnung ließ sich nicht korrekt abbilden, Nutzer behalf sich, indem er denselben Beleg manuell in zwei separate Ausgaben-Einträge aufteilte ("Gasthaus Horstmann 1/2", "…2/2") | `Ausgabe` hatte nur ein einzelnes `betragNetto`/`mwstSatz`-Paar — kein Feld für einen zweiten, gemischten Steuersatz auf demselben Beleg | Neue optionale Felder `Ausgabe.betragNetto2`/`mwstSatz2` (Migration `20260914123000_ausgabe_zweiter_mwst_satz`); in `/ausgaben/neu` und `/ausgaben/[id]` neue Checkbox "Beleg enthält einen zweiten MwSt-Satz" blendet ein zweites Betrag/Satz-Feldpaar ein, Live-Summenbox zeigt Netto/MwSt/Brutto über beide Anteile summiert (z.B. "MwSt 19%+7%"). Neue zentrale `ausgabeBetragsteile()`/`berechneAusgabeNetto()`/`berechneAusgabeMwst()`/`berechneAusgabeBrutto()` (`lib/utils.ts`) als einzige Quelle der Wahrheit für die Aufteilung — genutzt von `POST`/`PUT /api/ausgaben(/[id])` (Validierung 0\|7\|19 je Satz), `lib/datev.ts` (`sammleDatevBuchungen()` bucht zwei Zeilen mit gleichem Sach-/Gegenkonto, je einer pro Satz — DATEV kennt keinen gemischten Steuersatz auf einer Buchungszeile), `/api/exporte/ust-voranmeldung` (Vorsteuer-Aufteilung berücksichtigt beide Anteile), `/api/exporte/ausgaben` (PDF-Bericht, Spalte zeigt "19%+7%" bei gemischtem Satz), `/api/statistik(/ausgaben|/liquiditaet)` (Netto-/Brutto-Summen) und `app/finanzen/cashflow/page.tsx`. Bei Privatentnahme/-einlage bleibt der zweite Satz immer `null` (analog zur bestehenden MwSt-0%-Zwangsregel). 3 neue Unit-Tests in `__tests__/lib/utils.test.ts`; end-to-end gegen die echte DB + API + Playwright verifiziert: Schema-Round-Trip, DATEV-Export liefert für einen Testbeleg (31,01 € @19% + 20 € @7%) korrekt zwei Zeilen (36,90 €/19% und 21,40 €/7%, gleiches Sachkonto/Gegenkonto), UStVA-Vorsteuer korrekt auf beide Töpfe verteilt (5,89 € bzw. 1,40 €), ungültiger zweiter Satz wird mit 400 abgelehnt, Live-Formular zeigt korrekt "Netto 51,01 € / MwSt 19%+7% 7,29 € / Brutto 58,30 €" |
| KI-Belegerfassung in `/ausgaben/neu` und `/ausgaben/[id]`: Nutzer meldete "hat nicht funktioniert" — Button lief durch (Spinner), aber am Ende war kein Feld sichtbar befüllt und es erschien keine Fehlermeldung, wirkte wie ein kompletter Ausfall | `kiAnalyse()` zeigte bei HTTP 200 IMMER denselben generischen Erfolgstext ("Felder wurden automatisch ausgefüllt – bitte prüfen."), unabhängig davon, ob überhaupt etwas erkannt wurde. Liefert die KI bei einem schlecht lesbaren Beleg (Unschärfe, Thermodruck-Kassenbon, ungünstiger Ausschnitt) für `datum`/`belegNr`/`beschreibung`/`betragNetto` durchweg `null`, blieben ALLE Formularfelder auf ihrem Ausgangswert stehen (`kategorie`/`mwstSatz` fallen serverseitig ohnehin auf die bereits im Formular voreingestellten Defaults "Sonstige"/19 zurück) — für den Nutzer nicht von einem kompletten Ausfall zu unterscheiden. Zusätzlich nutzte die Seite das von `/api/ki/beleg` bereits gelieferte Feld `lieferant` nie, um `lieferantId` zu befüllen — anders als die strukturell identische KI-Erfassung in `/eingangsrechnungen/neu`, die genau das schon per Fuzzy-Match tut | `kiAnalyse()` in beiden Dateien sammelt jetzt, welche Felder tatsächlich einen Wert lieferten (`erkannt: string[]`), inkl. eines neuen Lieferanten-Fuzzy-Matches gegen die bereits geladene `lieferanten`-Liste (identisches Muster wie in `/eingangsrechnungen/neu`). Bei mindestens einem erkannten Feld erscheint jetzt eine konkrete Erfolgsmeldung ("Erkannt und übernommen: Datum, Betrag, Lieferant – bitte prüfen."), bei NULL erkannten Feldern eine klar abgesetzte, gelb statt lila eingefärbte Warnung ("Die KI konnte auf diesem Beleg keine Daten erkennen – bitte ein schärferes/helleres Foto versuchen oder die Felder manuell ausfüllen.") über einen neuen `kiWarnung`-State (auch bei echtem KI-Fehler/Exception gesetzt). End-to-end gegen die echte DB + Playwright verifiziert (`/api/ki/beleg`-Antwort gemockt): ein Ergebnis mit ausschließlich `null`-Feldern zeigt die gelbe Warnung mit korrekter CSS-Klasse; ein Ergebnis mit Datum/Beleg-Nr./Beschreibung/Betrag/Lieferant befüllt alle fünf Felder korrekt (inkl. `lieferantId` im Select, vorher nie verdrahtet) und zeigt die konkrete Erfolgsmeldung |
| Ausgabenbuch: Belege ließen sich nur EINZELN erfassen (Formular oder KI-Foto-Upload je ein Beleg) — Kundenwunsch: mit dem Handy unterwegs mehrere Belege nacheinander abfotografieren und sie gebündelt später am Rechner bearbeiten, analog zur bereits bestehenden Batch-Erfassung bei Lieferschein/Eingangsrechnung (`/ki/lieferung/batch`, `/eingangsrechnungen/batch`) | Für Ausgaben gab es nur den Einzel-Workflow (`kiAnalyse()` in `/ausgaben/neu`, siehe Zeile direkt darüber) — kein Sammel-Upload, keine Möglichkeit, mehrere Fotos zu machen und die Zuordnung/Prüfung zeitversetzt gebündelt vorzunehmen | Neue Modelle `KiAusgabenBatch`/`KiAusgabenBatchItem` (Migration `20260914170000_ki_ausgaben_batch`) + 5 neue Routen unter `/api/ki/ausgaben/batch(/[id]/…)` — 1:1 nach dem etablierten Muster von `KiLieferungBatch`/`KiEingangsrechnungBatch`: `POST /api/ki/ausgaben/batch` (multipart, bis 30 Dateien à max. 20 MB, jpg/png/webp/pdf) legt Batch + Items an und speichert die Dateien unter `getUploadBase()/ki-ausgaben-batch/<batchId>/`; `POST .../items` ergänzt weitere Fotos (setzt einen bereits "bereit"-Batch zurück auf "offen"); `POST .../[id]/analyze` ruft `analyzeDocument()`/Mistral-OCR mit dem bestehenden `PROMPTS.beleg` auf (`P.KI_NUTZEN`-gated) und leitet fehlenden Netto-Betrag per bereits vorhandenem `berechneNettoAusBrutto()` (siehe PR zu 13,01-€-Waschpark-Fall weiter oben) aus Brutto+MwSt ab, statt der KI die Divisionsarithmetik zu überlassen; `PATCH .../items/[itemId]` übernimmt Korrekturen inkl. optionalem zweitem MwSt-Satz (`betragNetto2`/`mwstSatz2`, siehe Zeile zwei weiter oben); `PUT .../[id]` mit `{aktion:"verwerfen"}` löscht Batch+Dateien, mit `{aktion:"abschliessen", itemId?}` legt für jedes Item mit `entscheidung:"passt"` eine echte `Ausgabe` an (Sachkonto via `getSachkonto()`, Lieferant per bereits vorhandenem Fuzzy-Match) und verschiebt die Beleg-Datei vom Batch-Ablagepfad in die reguläre `public/uploads/belege/<jahr>/…`-Struktur (identisches Namensschema wie der bestehende Einzel-Upload in `/api/ausgaben/[id]/beleg`), inkl. Best-effort-Spiegelung nach Nextcloud. Frontend: `/ausgaben/neu` bekommt einen Modus-Umschalter (Einzeln/Batch, `?modus=batch`) — der Batch-Modus nutzt `MultiCameraUpload` (bereits geteilte Komponente aus Lieferung/Eingangsrechnung-Batch, unverändert wiederverwendet) für serielles Fotografieren und zeigt eine Übersicht offener Batches; neue Seite `/ausgaben/batch/[id]` (Review-UI nach dem Vorbild von `/eingangsrechnungen/batch/[id]`) lässt pro Beleg das KI-Ergebnis prüfen/korrigieren (Lieferant **optional**, anders als bei Eingangsrechnung — nur Beschreibung + Betrag sind Pflicht) und einzeln oder gesammelt übernehmen, mit Live-Summenbox (`berechneAusgabeNetto/Mwst/Brutto`) bei aktiviertem zweitem MwSt-Satz. End-to-end gegen die echte DB + Playwright verifiziert (Batch mit 2 Fotos, KI-Antwort gemockt): vollständig erkanntes Item zeigt korrekt Lieferanten-Fuzzy-Match + "Passt", unvollständiges Item zeigt "Beschreibung fehlt"; Einzel-Übernahme legt korrekt eine `Ausgabe` mit richtigem Sachkonto und am regulären Pfad abgelegter Beleg-Datei an; Sammel-Übernahme des zweiten Batches sowie Verwerfen eines dritten, leeren Batches ebenfalls verifiziert |
| Mitarbeiter-Stammdaten (`/personal/[id]`, `/personal/neu`) hatten kein Feld für die private Anschrift — nur Kontakt (E-Mail/Telefon) und Bankverbindung waren erfassbar | Fehlendes Feld im Datenmodell — `Mitarbeiter` kannte anders als `Kunde`/`Lieferant` keine `strasse`/`plz`/`ort`-Felder | Neue optionale Felder `Mitarbeiter.strasse`/`plz`/`ort` (Migration `20260915090000_mitarbeiter_anschrift`, gleiche Feldnamen wie bei `Kunde`/`Lieferant`); `POST`/`PUT /api/personal/mitarbeiter(/[id])` nehmen sie entgegen, `/personal/neu` zeigt sie im Kontakt-Abschnitt, `/personal/[id]` (Stammdaten-Tab, Bearbeiten-Modus) ergänzt sie in der Kontakt-Karte, Lese-Ansicht zeigt Straße sowie "PLZ Ort" zusammengefasst. `GET /api/personal/mitarbeiter/me` (Selbstbedienungs-Route) bewusst NICHT erweitert — liefert weiterhin nur die für die Arbeitszeiterfassung nötigen Felder, keine Adressdaten. End-to-end gegen die echte DB verifiziert: `POST`/`PUT` speichern und liefern Straße/PLZ/Ort korrekt zurück, `GET` danach ebenfalls konsistent |
| Personal-Selbstbedienung (`Benutzer.mitarbeiterId`-verknüpfte Logins) konnte nur die eigene Arbeitszeit erfassen (`/meine-arbeitszeit`) — Kundenwunsch: Mitarbeiter sollen ihre eigenen Lohnabrechnungen/Gehaltszettel selbst einsehen und ausdrucken können, ohne dass jemand sie manuell aushändigen muss | `GET /api/personal/abrechnungen(/[id])` war komplett hinter `requireVollePersonalRechte()` gesperrt (403 für jeden Selbstbedienungs-Account, siehe Kommentar dort "…alles außer der eigenen Arbeitszeiterfassung: … Gehaltsabrechnungen …") — kein Weg, den Zugriff auf die EIGENEN Abrechnungen aufzuweiten, ohne versehentlich auch Kollegendaten freizugeben | Beide `GET`-Handler (Liste + Detail) bekommen denselben Dual-Modus wie `arbeitsstunden` (`istPersonalSelbstbedienung()` statt hartem 403): die Liste filtert bei Selbstbedienung zwingend auf `mitarbeiterId = me.mitarbeiterId` UND `status ∈ {ABGERECHNET, AUSGEZAHLT}` — ein mitgesendeter `mitarbeiterId`/`status`-Query-Parameter wird dabei ignoriert (sonst könnte ein Account per URL eine Kollegen-ID oder den `OFFEN`-Status erzwingen); ein `OFFEN`-Entwurf bleibt bewusst unsichtbar, da sich Brutto/Netto darin noch ändern können und ein Mitarbeiter keine vorläufigen Zahlen sehen soll. Die Detail-Route liefert bei fremder `mitarbeiterId` oder Status `OFFEN` bewusst **404** statt 403 (bestätigt nicht, dass die ID überhaupt existiert). `PUT`/`DELETE` bleiben unverändert komplett gesperrt (`requireVollePersonalRechte()`) — Selbstbedienung darf nur lesen. Neue Seite `/meine-lohnabrechnungen` (Liste mit Zeitraum/Netto/Status/Zahlungsdatum + Link "Ansehen/Drucken") verlinkt auf die bereits bestehende Druckseite `/personal/abrechnungen/[id]/druck` (nutzt jetzt automatisch denselben Dual-Modus-Zugriff, keine Änderung an der Seite selbst nötig). `components/Nav.tsx`: die bisher fest auf genau einen Pfad (`/meine-arbeitszeit`) verdrahtete Selbstbedienungs-Redirect-Logik erlaubt jetzt mehrere Pfade (`SELBSTBEDIENUNG_HREFS` + Regex-Ausnahme für `/personal/abrechnungen/<id>/druck`, rein kosmetisch — die eigentliche Absicherung sitzt in der API), neuer Nav-Link "Meine Lohnabrechnungen" neben "Meine Arbeitszeit". End-to-end gegen die echte DB + Playwright verifiziert (Selbstbedienungs-Account testweise sogar mit `rolle:"admin"` angelegt, um zu beweisen dass die `mitarbeiterId`-Verknüpfung Vorrang hat): Liste zeigt korrekt nur die eigenen 2 finalisierten Abrechnungen, ein mitgeschickter `?mitarbeiterId=<kollege>&status=OFFEN`-Parameter wird ignoriert, die eigene `OFFEN`-Abrechnung liefert einzeln 404, die Abrechnung eines Kollegen liefert 404, `PUT` bleibt mit 403 gesperrt; die Druckseite zeigt den eigenen Gehaltszettel korrekt an und liefert für eine fremde ID "nicht gefunden"; ein Navigationsversuch auf `/personal` wird weiterhin zuverlässig auf `/meine-arbeitszeit` zurückgeleitet |
| Nachzieher zur Zeile direkt darüber: Missverständnis aufgeklärt — der Kundenwunsch war von Anfang an, das ECHTE, vom Steuerberater gelieferte Original-PDF hochladen und den Mitarbeitern zum Selbst-Ausdrucken zur Verfügung stellen zu können, NICHT die intern aus den DB-Werten generierte Druckseite (die nur eine grobe Eigen-Rekonstruktion ist, kein offizieller Gehaltszettel) | `Gehaltsabrechnung` hatte kein Feld für eine hochgeladene Datei — anders als `Ausgabe.belegPfad`/`EingangsRechnung.belegpfad` gab es für Lohnabrechnungen überhaupt keinen Upload-Mechanismus, weder Feld noch Route noch UI | Neue Felder `Gehaltsabrechnung.belegPfad`/`belegDateiname` (Migration `20260916120000_gehaltsabrechnung_beleg`). Neue Route `app/api/personal/abrechnungen/[id]/beleg/route.ts`: `POST`/`DELETE` nur mit vollen Personal-Rechten (`requireVollePersonalRechte()` — Selbstbedienung darf nicht hochladen), nur `.pdf` bis 20 MB, Ablage unter `getUploadBase()/personal-abrechnungen/`. Bewusst **kein** Wiederverwenden des generischen `/api/uploads/[...path]` (das jeden eingeloggten Nutzer bedient, ohne zu prüfen WESSEN Dokument angefragt wird) — Gehaltsdaten sind sensibler als ein Eingangsrechnungs-Beleg, deshalb eigener `GET`-Handler direkt in derselben Route: prüft pro Aufruf wie `GET /api/personal/abrechnungen/[id]` (Selbstbedienung nur eigene, nicht-OFFEN Abrechnung, sonst 404 statt 403), erst danach wird die Datei gestreamt (`Content-Disposition: inline`, `Cache-Control: private, no-store`). `DELETE /api/personal/abrechnungen/[id]` löscht eine vorhandene Beleg-Datei beim Löschen der Abrechnung mit (verhindert verwaiste Dateien). Admin-Tab (`/personal/[id]` → Abrechnung) bekommt je Zeile "PDF hochladen"/"Ersetzen"/"PDF entfernen" neben "Druck" (Aktionen-Zelle jetzt `flex-wrap`, da mehr Buttons als vorher). `/meine-lohnabrechnungen` verlinkt jetzt NICHT mehr auf die Druckseite, sondern auf `GET .../beleg` — ist noch kein PDF hinterlegt, zeigt die Zeile "Noch nicht hinterlegt" statt eines Links (bewusst kein Rückfall auf die interne Druckseite, wie vom Kunden gewünscht). Die Druckseite selbst (`/personal/abrechnungen/[id]/druck`) bleibt unverändert bestehen und bleibt im Admin-Tab weiterhin verlinkt (internes Hilfsmittel, z.B. bevor ein PDF vorliegt) |
| Nutzer suchte den neuen "PDF hochladen"-Button (siehe zwei Zeilen weiter oben) auf der allgemeinen Liste `/personal/abrechnungen` statt auf der Mitarbeiter-Detailseite — auf dem Handy war dort zusätzlich die komplette "Aktionen"-Spalte (Auszahlen/Druck/Löschen) unsichtbar/unerreichbar, obwohl der Spaltenkopf im Markup vorhanden war | `<table>` auf `app/personal/abrechnungen/page.tsx` war direkt in eine Karte mit `overflow-hidden` eingebettet, ohne eigenen `overflow-x-auto`-Wrapper — bei 390px reicht der Platz für Typ/Brutto/Netto/Status/Aktionen nicht, `overflow-hidden` schnitt die Aktionen-Spalte dadurch komplett ab statt sie scrollbar zu machen. Exakt derselbe Bug-Mechanismus, der bereits bei `/gutschriften/[id]` und den drei Tabellen auf `/personal/[id]` behoben wurde (siehe Zeilen weiter oben) — auf dieser vierten, separaten Listen-Seite aber nie mitgezogen, weil das PDF-Upload-Feature nur den Abrechnung-Tab der Mitarbeiter-Detailseite betraf, nicht diese cross-Mitarbeiter-Übersicht (die ohnehin nie PDF-Upload bekommt, nur Auszahlen/Druck/Löschen) | Tabelle in `app/personal/abrechnungen/page.tsx` in einen `<div className="overflow-x-auto">` innerhalb der Karte gewrappt (äußeres `overflow-hidden` bleibt für die abgerundeten Ecken erhalten) — identisches, bereits etabliertes Muster. Kein PDF-Upload auf dieser Seite ergänzt (bewusst nicht angefragt) — stattdessen klargestellt: der Mitarbeitername in der ersten Spalte ist bereits ein Link auf `/personal/[id]?tab=abrechnung`, wo der Upload sitzt. End-to-end gegen die echte DB + Playwright mit mobilem Viewport (390px) verifiziert: Scroll-Container ist jetzt horizontal scrollbar (`scrollWidth 516px > clientWidth 356px`), nach dem Scrollen sind "Druck"/"Löschen" sichtbar, kein Overflow der gesamten Seite (`document.documentElement.scrollWidth === clientWidth`) |
| Nachzieher: die Aktionen im Abrechnung-Tab (`/personal/[id]`) waren reine `text-xs ... hover:underline`-Textlinks ohne Rahmen/Hintergrund — auf dem Handy war das Wort schwer zu treffen (kleine, ungepolsterte Tap-Fläche) | Kein visuelles Tap-Target — nur die Textzeichen selbst waren klickbar, keine Polsterung/Umrandung, die die tatsächliche Trefferfläche vergrößert | Alle Aktionen (Bearbeiten/Speichern/Abbrechen/Abrechnen/Auszahlen/Druck/📄 PDF/Ersetzen/PDF entfernen/PDF hochladen/Löschen) auf das im Projekt bereits etablierte Pill-Button-Muster umgestellt (`text-xs px-2.5 py-1.5 bg-{farbe}-50 hover:bg-{farbe}-100 text-{farbe}-700 border border-{farbe}-200 rounded-lg transition-colors`, z.B. schon in `app/kunden/[id]/page.tsx`/`app/kampagnen/[id]/page.tsx` verwendet) statt reiner Unterstreichung — Farbcodierung nach Bedeutung beibehalten (grün=Geld/Bestätigen, blau=Bearbeiten, grau=neutral, rot=Löschen/Entfernen). Bewusst NUR auf `/personal/[id]` (Abrechnung-Tab) angewendet, NICHT auf die separate Liste `/personal/abrechnungen` (dort weiterhin reine Textlinks, nicht Teil dieser Anfrage). End-to-end gegen die echte DB + Playwright mit mobilem Viewport (390px) verifiziert: alle Buttons rendern mit sichtbarem Rahmen/Hintergrund und ca. 30px Höhe (vorher reine Textzeile) statt purer Unterstreichung |
| Ausgabenbuch (`/ausgaben/neu`, `/ausgaben/[id]`): ein Kassenbon-Netto-Betrag mit 3 Nachkommastellen (z.B. Tankquittung "10,924 €" netto, damit 10,924 × 1,19 = 13,00 € exakt zum aufgedruckten Bruttobetrag passt — mit nur 2 Nachkommastellen hätte 10,92 × 1,19 = 12,99 € gerechnet, einen Cent daneben) ließ sich zwar eintippen und die Live-Vorschau rechnete auch schon korrekt damit, aber "Speichern" tat scheinbar gar nichts — kein Fehlertext, kein Request, das Formular blieb einfach stehen | Das `<input type="number" step="0.01" min="0" ...>`-Feld für "Betrag netto (€)" (und "Betrag netto 2") steht in einem `<form>` ohne `noValidate`; ein Wert mit 3 Nachkommastellen erzeugt eine native `stepMismatch`-Ungültigkeit (`0.924 / 0.01` ist nicht ganzzahlig). Beim Klick auf "Speichern" (`type="submit"`) prüft der Browser das VOR dem React-`onSubmit`-Handler und bricht dabei lautlos ab (nur ein optisches natives Tooltip, kein von der App kontrollierter Fehlertext) — der `fetch()` zu `POST /api/ausgaben` feuert nie. Die Batch-Erfassung (`/ausgaben/batch/[id]`) nutzt für dieselben Felder die eigene `DezimalInput`-Komponente (kein natives `step`) und war nicht betroffen | `step="0.01"` auf `step="0.001"` angehoben, für "Betrag netto" UND "Betrag netto 2" in beiden Formularen (`/ausgaben/neu`, `/ausgaben/[id]`) — damit ist ein Wert mit bis zu 3 Nachkommastellen jetzt formal gültig und das Formular sendet wie erwartet. Kein Runden auf 2 Nachkommastellen beim Speichern ergänzt: genau das hätte den ursprünglichen Anwendungsfall (Bon-Netto mit 3 Nachkommastellen, damit die MwSt-Berechnung exakt auf den aufgedruckten Bruttobetrag kommt) wieder zunichtegemacht. Per Playwright gegen die echte laufende App verifiziert (`checkValidity()`/`validity.stepMismatch` direkt am Feld geprüft): vor dem Fix `stepMismatch:true`, kein POST an `/api/ausgaben`; nach dem Fix `stepMismatch:false`, POST feuert und die Ausgabe landet korrekt mit `betragNetto:10.924` in der DB |
| Ausgabenbuch: neu angelegte Ausgaben-Kategorien ("Tanken", "Porto") bekamen beim DATEV-Export ein falsches/generisches Sachkonto zugewiesen, und eine Ausgabe mit Buchungstyp "Bewirtung" landete konsequent auf Konto 4654 statt des von der Steuerberaterin erwarteten 4650 | Zwei unabhängige Ursachen, die beide wie "falsches Konto" aussahen: (1) `getSachkonto()` (`lib/datev.ts`) kennt nur ein festes Kategorie→Konto-Mapping (`SACHKONTEN_SKR03`/`SKR04`, z.B. Fahrtkosten→4530) plus die überschreibbare `ausgaben.sachkonten`-Einstellung (`/einstellungen/ausgaben`, Abschnitt "Buchungskonto-Zuordnung") — eine über "+ Hinzufügen" neu angelegte Kategorie wie "Tanken"/"Porto" landet in KEINEM der beiden und fällt auf den Sonstige-Fallback (4900 SKR03/6800 SKR04) zurück, bis für sie explizit ein Sachkonto hinterlegt wird. (2) Bewirtung ist kein Kategorie-, sondern ein **Buchungstyp** (fester Dropdown Betriebsausgabe/Privatentnahme/Privateinlage/Reisekosten/Bewirtung) — dessen Konto kommt aus `BUCHUNGSTYP_KONTEN_SKR03`/`SKR04` und hat laut `getSachkonto()`-Priorität IMMER Vorrang vor Kategorie-Mapping und der Einstellungen-Tabelle, ist also unabhängig von Sektion 2 der Ausgaben-Einstellungen und lässt sich dort nicht korrigieren. Der hinterlegte Wert 4654 (SKR03) war schlicht falsch: Bewirtung wird hier als EINE Buchungszeile mit vollem Bruttobetrag + BU-Schlüssel 9 gebucht (`getBuSchluessel()`) — DATEV/Steuerberater-Software splittet die nicht abzugsfähigen 30 % anhand des BU-Schlüssels automatisch, und genau dafür ist 4650 ("Kosten der Bewirtung aus geschäftlichem Anlass") das richtige SKR03-Konto; 4654 wäre nur korrekt, wenn der nicht abzugsfähige Anteil manuell als eigene Zeile OHNE BU-Schlüssel gebucht würde. Der zugehörige SKR04-Wert (6640) war bereits richtig gesetzt (entspricht per gleichem Versatz wie bei Fahrtkosten 4530↔6520 exakt 4650, nicht 4654) | Für Tanken/Porto: **reine Einstellungssache**, kein Code-Fix nötig — unter `/einstellungen/ausgaben` im Abschnitt "Buchungskonto-Zuordnung (Sachkonten)" je Kategorie das gewünschte Konto eintragen (z.B. Tanken→4530, Porto→4910) und speichern; wirkt sofort auf neue Ausgaben UND den nächsten DATEV-Export. Für Bewirtung: `BUCHUNGSTYP_KONTEN_SKR03.Bewirtung` in `lib/datev.ts` von `"4654"` auf `"4650"` korrigiert (SKR04 unverändert `"6640"`, war bereits korrekt) — das ist der hartkodierte Vorschlag sowohl in der Live-Sachkonto-Vorschau in `/ausgaben/neu`/`/ausgaben/[id]` als auch im tatsächlichen DATEV-Export (`sammleDatevBuchungen()`), da beide dieselbe Konstante importieren; Hinweistext in `/einstellungen/ausgaben` ("Typ-basierte Konten … Bewirtung=4654 …") entsprechend auf 4650 korrigiert. Buchungstyp-Konten sind bewusst weiterhin NICHT über die Einstellungen-Tabelle konfigurierbar (nur Kategorie-Konten sind es) — das betrifft nur die vier Buchungstypen mit steuerlicher Sonderbehandlung (Privatentnahme/-einlage/Reisekosten/Bewirtung), keine neue Anfrage, weiterer Ausbau bei Bedarf möglich. Per `getSachkonto()`-Direktaufruf verifiziert: Bewirtung liefert jetzt `4650` (SKR03)/`6640` (SKR04) unverändert, Fahrtkosten/Reisekosten weiterhin `4530`, eine unregistrierte Kategorie weiterhin `4900` (Fallback bleibt bewusst bestehen — sonst müsste jede neu erfundene Kategorie sofort ein Konto haben) |

## Schemata: Wichtige Felder

- `Gehaltsabrechnung.belegPfad String?` + `belegDateiname String?` — vom Steuerberater hochgeladenes Original-PDF (relativer Pfad unter `getUploadBase()`, siehe `/api/personal/abrechnungen/[id]/beleg`); Personal-Selbstbedienung darf es nur für die eigene, nicht-OFFENE Abrechnung herunterladen
...
- `Ausgabe.betragNetto2 Float?` / `Ausgabe.mwstSatz2 Float?` — optionaler zweiter Netto-/MwSt-Anteil für Belege mit gemischten Sätzen auf EINER Rechnung (z.B. Bewirtung: Speisen 7 % / Getränke 19 %); beide nur gesetzt, wenn der Nutzer im Formular "Beleg enthält einen zweiten MwSt-Satz" aktiviert hat. Einzige Quelle der Wahrheit für die Zerlegung ist `ausgabeBetragsteile()`/`berechneAusgabeNetto()`/`berechneAusgabeMwst()`/`berechneAusgabeBrutto()` (`lib/utils.ts`) — DATEV-Export bucht dadurch zwei Zeilen (eine je Satz, gleiches Sach-/Gegenkonto), UStVA-Export addiert beide Anteile in den jeweiligen Vorsteuer-Topf. Bei `buchungstyp` Privatentnahme/Privateinlage bleibt der zweite Satz immer `null` (analog zur MwSt-0%-Zwangsregel)
