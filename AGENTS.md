...
| Rechnungs-PDF ("PDF"-Button, `downloadVorschauPdf()` in `app/lieferungen/[id]/rechnung/page.tsx`): auf dem Handy erzeugt verteilte sich dieselbe Rechnung auf deutlich mehr Seiten (Testfall: 15 statt 2) als am Desktop | `[data-print-area]` hat nur `maxWidth: "210mm"`, kein festes `width` — auf Bildschirmen schmaler als ~794px (jedes Handy-Viewport) schrumpft das Element dadurch auf Viewport-Breite statt A4-Breite. Die Positionstabelle bricht dadurch viel stärker um, der html2canvas-Screenshot wird entsprechend höher, und die anschließende Aufteilung in A4-Seiten (`pageH`-Schritte) erzeugt dadurch weit mehr Seiten als am Desktop, wo der Screenshot bereits die korrekten A4-Proportionen hat | Vor jeder html2canvas-Aufnahme (Hauptinhalt UND der separate Fußzeilen-Screenshot in `downloadVorschauPdf()`, sowie der einfachere Screenshot in `getInhalt` des `NextcloudUploadButton`) wird `element.style.width` auf eine feste `A4_BREITE_PX = "793.7px"` (210mm bei 96dpi) und `maxWidth` auf `"none"` gesetzt, danach in `finally` auf den ursprünglichen Wert zurückgesetzt — analog zum bereits bestehenden Muster, die Fußzeile per `style.display` kurzzeitig auszublenden. Die reine Bildschirmansicht bleibt weiterhin responsive (kein horizontales Scrollen beim Betrachten auf dem Handy), nur die für den Export erzeugten Screenshots werden auf A4-Breite fixiert. Per Playwright mit mobilem Viewport (390px) end-to-end verifiziert: vorher 15 Seiten, danach 2 Seiten (identisch zum Desktop-Ergebnis). Betrifft nur `/lieferungen/[id]/rechnung`; derselbe `maxWidth:"210mm"`-ohne-`width`-Mustercode existiert auch in `app/angebote/[id]/druck/page.tsx`, `app/lieferungen/[id]/lieferschein/page.tsx` und `app/vorbestellungen/[id]/auftragsbestaetigung/page.tsx` (dort noch nicht behoben, da nicht gemeldet) |
