...
export async function GET() {
  try {
    const batches = await prisma.kiLieferungBatch.findMany({
    ...
    console.error("KiLieferungBatch GET error:", e);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const batch = await prisma.kiLieferungBatch.create({ data: {} });
    const uploadDir = path.join(getUploadBase(), "ki-lieferung-batch", String(batch.id));
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const item = await prisma.kiLieferungBatchItem.create({
        data: {
          batchId: batch.id,
          reihenfolge: i,
          dateiPfad: `ki-lieferung-batch/${batch.id}/${filename}`,
          dateiName: file.name,
        },
      });
      items.push(item);
    ...
    Sentry.captureException(e);
    console.error("KiLieferungBatch POST error:", e);
