...
export async function GET() {
  try {
    const batches = await prisma.kiAusgabenBatch.findMany({
    ...
    console.error("KiAusgabenBatch GET error:", e);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const batch = await prisma.kiAusgabenBatch.create({ data: {} });
    const uploadDir = path.join(getUploadBase(), "ki-ausgaben-batch", String(batch.id));
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const item = await prisma.kiAusgabenBatchItem.create({
        data: {
          batchId: batch.id,
          reihenfolge: i,
          dateiPfad: `ki-ausgaben-batch/${batch.id}/${filename}`,
          dateiName: file.name,
        },
      });
      items.push(item);
    ...
    Sentry.captureException(e);
    console.error("KiAusgabenBatch POST error:", e);
