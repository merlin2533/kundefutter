...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    try {
      ...
      // fehlt es, deterministisch aus dem Bruttobetrag ableiten statt der KI eine Kopfrechnung
      // aufzuerlegen (siehe lib/utils.ts berechneNettoAusBrutto()).
      const betragNetto =
      ...
      const updated = await prisma.kiAusgabenBatchItem.update({
        where: { id: itemId },
        data: {
          status: "analysiert",
    ...
    Sentry.captureException(e);
    console.error("KiAusgabenBatch analyze error:", e);
