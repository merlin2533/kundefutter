...
export default function RechnungenPage() {
  ...
  const [loeschRechnung, setLoeschRechnung] = useState<Rechnung | null>(null);
  ...
  async function rechnungLoeschen(begruendung: string) {
    if (!loeschRechnung) return;
    setLoeschLoading(true);
    setLoeschError(undefined);
    try {
      const res = await fetch(`/api/lieferungen/${loeschRechnung.id}`, {
        method: "PATCH",
  ...
  return (
    <main className="max-w-screen-xl mx-auto px-4 py-4 sm:py-8">
                    ...
                    >
                      ...
                      <td className="px-4 py-3 hidden sm:table-cell" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center gap-0.5">
                          ...
                          {/* Rechnung endgültig löschen – nur Admins */}
                          {istAdmin && (
                            <button
                              onClick={() => { setLoeschError(undefined); setLoeschRechnung(r); }}
                              className="p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-700 rounded transition-colors"
      ...
      <RechnungLoeschenModal
        open={loeschRechnung !== null}
        rechnungNr={loeschRechnung?.rechnungNr ?? ""}
        loading={loeschLoading}
        error={loeschError}
        onClose={() => { if (!loeschLoading) setLoeschRechnung(null); }}
        onConfirm={rechnungLoeschen}
      />
    </main>
  );
}
