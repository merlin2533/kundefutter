    ...
    <>
      ...
      <div className="print-hidden sticky top-0 z-20 flex items-center flex-wrap gap-1.5 p-2.5 bg-white/95 backdrop-blur border-b border-gray-200 shadow-sm">
        <button
          ...
          // Anlegen einer Lieferung landet man hier über einen router.push() von
          // /lieferungen/neu — history.back() würde dann auf das (jetzt leere)
          // Neu-Formular zurückspringen statt auf die soeben erfasste Lieferung.
      ...
      <td style={{ padding: 0, border: "none" }}>
        ...
        <div style={{ marginBottom: "32px" }}>
          ...
          <div style={{ fontWeight: "bold", fontSize: "12pt" }}>{kunde.firma ?? kunde.name}</div>
          {kunde.firma && <div style={{ fontSize: "10pt" }}>{kunde.name}</div>}
