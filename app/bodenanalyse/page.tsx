      ...
      <tbody>
        {bewertung.map((b) => (
          <tr key={b.parameter} className={b.status === "kritisch" ? "bg-red-50" : b.status !== "ok" ? "bg-amber-50" : ""}>
            ...
            <td className="py-1 text-gray-600 leading-snug">{b.ist !== null ? b.hinweis : "—"}</td>
          </tr>
        ))}
      </tbody>
    </table>
...
export default function BodenanalyseListePage() {
  ...
  const gefiltert = analysen.filter((a) => {
    if (filterKunde && !a.schlag.kunde.name.toLowerCase().includes(filterKunde.toLowerCase())) return false;
    if (filterSchlag && !a.schlag.name.toLowerCase().includes(filterSchlag.toLowerCase())) return false;
    return true;
  });
  ...
  return (
    <div>
      ...
      <div className="mb-4 rounded-xl border border-amber-300 bg-amber-50 overflow-hidden">
        ...
        >
          <span className="text-sm font-semibold text-amber-800">⚠ Wichtiger Hinweis (Albrecht-Methode)</span>
          <span className="text-amber-600 text-xs">{disclaimerOpen ? "▲ Einklappen" : "▼ Ausklappen"}</span>
        </button>
        {disclaimerOpen && (
          <p className="px-4 pb-3 text-xs text-amber-700 leading-relaxed">{ALBRECHT_DISCLAIMER}</p>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <input
          className="w-full sm:w-56 border border-gray-300 rounded-lg px-3 py-2 text-sm"
      ...
      {!loading && !error && gefiltert.length === 0 && (
        <div className="text-center py-12 text-gray-400">
          ...
          <p className="text-sm">Noch keine Albrecht-Analysen erfasst.</p>
          <Link href="/bodenanalyse/neu" className="text-green-700 hover:underline text-sm mt-2 inline-block">
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium text-gray-600 hidden sm:table-cell">Kunde</th>
                <th className="px-4 py-3 text-right font-medium text-gray-600 hidden md:table-cell">pH</th>
                <th className="px-4 py-3 text-right font-medium text-gray-600 hidden md:table-cell">KAK</th>
                <th className="px-4 py-3 text-center font-medium text-gray-600">Ca% Mg% K% H%</th>
                <th className="px-4 py-3 text-center font-medium text-gray-600">Bewertung</th>
                <th className="px-4 py-3 text-center font-medium text-gray-600 hidden lg:table-cell">Beleg</th>
                  ...
                  <>
                    ...
                    >
                      <td className="px-4 py-3 text-gray-800 font-medium whitespace-nowrap">
                        ...
                        <div className="sm:hidden text-xs text-gray-400 mt-0.5">{a.schlag.kunde.name}</div>
                      </td>
                      <td className="px-4 py-3 text-gray-700">
                        {a.schlag.name}
                        {a.kultur && <span className="ml-1 text-xs text-gray-400">({a.kultur})</span>}
                      </td>
                      <td className="px-4 py-3 text-gray-600 hidden sm:table-cell">
                        <Link
                          href={`/kunden/${a.schlag.kundeId}?tab=Albrecht`}
                      ...
                      <td className="px-4 py-3 text-right tabular-nums text-gray-700 hidden md:table-cell">
                        {a.phH2O !== null ? a.phH2O.toFixed(1) : "—"}
                      </td>
                      <td className="px-4 py-3 text-right tabular-nums text-gray-700 hidden md:table-cell">
                        {a.kak !== null ? a.kak.toFixed(1) : "—"}
                      </td>
                      ...
                      <td className="px-4 py-3 text-center hidden lg:table-cell">
                        {a.belegPfad ? (
                          <a href={a.belegPfad} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline text-xs" onClick={(e) => e.stopPropagation()}>
                            {a.belegName ?? "Beleg"}
                          </a>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                              ...
                              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-700">
                                {a.probenId && <><span className="text-gray-500">Proben-ID:</span><span>{a.probenId}</span></>}
                                ...
                                {a.nGesamt !== null && <><span className="text-gray-500">N-Gesamt:</span><span>{a.nGesamt} %</span></>}
                                ...
                                {a.caCo3 !== null && <><span className="text-gray-500">CaCO₃:</span><span>{a.caCo3} %</span></>}
                                {a.leitfaehigkeit !== null && <><span className="text-gray-500">Leitfähigkeit:</span><span>{a.leitfaehigkeit} mS/cm</span></>}
                              </div>
                            </div>
                            <div>
                              ...
                              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-700">
                                ...
                                {a.schwefel !== null && <><span className="text-gray-500">Schwefel:</span><span>{a.schwefel} ppm</span></>}
                                {a.p2o5Verfuegbar !== null && <><span className="text-gray-500">P₂O₅ verf.:</span><span>{a.p2o5Verfuegbar} kg/ha</span></>}
                                {a.p2o5Vorrat !== null && <><span className="text-gray-500">P₂O₅ Vorrat:</span><span>{a.p2o5Vorrat} kg/ha</span></>}
                              </div>
                            </div>
                          </div>
