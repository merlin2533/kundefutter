...
function PreisChart({ data, fmt, fmtD }: {
  data: PreishistorieEntry[];
  fmt: (v: number) => string;
  fmtD: (v: string) => string;
}) {
  ...
  const toY = (p: number) => PT + (1 - (p - minY) / (maxY - minY)) * plotH;
  const pts = series.map((s, i) => ({ x: toX(i), y: toY(s.price), ...s }));
  const poly = pts.map((p) => `${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(" ");
  const diff = series[series.length - 1].price - series[0].price;
  const color = diff > 0 ? "#dc2626" : diff < 0 ? "#16a34a" : "#6b7280";
  const yTicks = [0, 1, 2, 3].map((i) => ({ y: toY(minY + (i / 3) * (maxY - minY)), p: minY + (i / 3) * (maxY - minY) }));
  const step = Math.max(1, Math.ceil(series.length / 6));
  const xIdxs = series.reduce<number[]>((a, _, i) => { if (i % step === 0 || i === series.length - 1) a.push(i); return a; }, []);
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ maxHeight: 220 }} className="select-none">
      {yTicks.map((t, i) => (
        <g key={i}>
          <line x1={PL} y1={t.y} x2={W - PR} y2={t.y} stroke="#f3f4f6" strokeWidth="1" />
          <text x={PL - 6} y={t.y + 4} textAnchor="end" fontSize="10" fill="#9ca3af">{fmt(t.p)}</text>
        </g>
      ))}
      <polygon points={`${PL},${PT + plotH} ${poly} ${pts[pts.length - 1].x.toFixed(1)},${(PT + plotH).toFixed(1)}`} fill={color} opacity="0.07" />
      <polyline points={poly} fill="none" stroke={color} strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" />
      {pts.map((p, i) => (
        <g key={i}><circle cx={p.x} cy={p.y} r="4.5" fill="white" stroke={color} strokeWidth="2" /><title>{fmtD(p.date)}: {fmt(p.price)}</title></g>
      ))}
      {xIdxs.map((idx) => (
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {lagerRelevant && (
          <div className={`rounded-xl border p-3 shadow-sm ${status === "rot" ? "bg-red-50 border-red-200" : status === "gelb" ? "bg-amber-50 border-amber-200" : "bg-white border-gray-200"}`}>
            ...
            <p className={`text-xl font-bold ${status === "rot" ? "text-red-700" : status === "gelb" ? "text-amber-700" : "text-gray-900"}`}>
              {artikel.aktuellerBestand.toLocaleString("de-DE")}<span className="text-sm font-normal text-gray-500 ml-1">{artikel.einheit}</span>
            </p>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6 max-w-2xl">
          ...
          {editing ? (
            ...
            <div>
              {/* Stammdaten */}
              <div className="mb-5">
                <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Stammdaten</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 border-t border-gray-100">
                  {[
                    { label: "Artikelnummer", value: <span className="font-mono">{artikel.artikelnummer}</span> },
                    { label: "Kategorie", value: artikel.kategorie === "Duenger" ? "Dünger" : artikel.kategorie },
                    { label: "Einheit", value: artikel.einheit },
                    { label: "Status", value: artikel.aktiv
                      ? <span className="text-xs px-2 py-0.5 bg-green-100 text-green-800 border border-green-200 rounded font-medium">Aktiv</span>
                      : <span className="text-xs px-2 py-0.5 bg-gray-100 text-gray-600 border border-gray-200 rounded font-medium">Inaktiv</span>
                    },
                  ].map(({ label, value }) => (
                    <div key={label} className="py-2.5 flex gap-3 border-b border-gray-100">
              ...
              {/* Preise */}
              <div className="mb-5">
                <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Preise & Steuern</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 border-t border-gray-100">
                  {[
                    { label: "Standardpreis", value: <span className="font-mono font-semibold">{formatEuro(artikel.standardpreis)}</span> },
                    { label: "MwSt-Satz", value: artikel.mwstSatz === 0 ? "0% (Steuerfrei)" : artikel.mwstSatz === 7 ? "7% (ermäßigt)" : "19% (Regelsatz)" },
                    { label: "Preisstand", value: artikel.preisStand ? formatDatum(artikel.preisStand) : "—" },
              ...
              {lagerRelevant && (
                <div className="mb-5">
                  <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Lager</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 border-t border-gray-100">
                    {[
                      { label: "Bestand", value: `${artikel.aktuellerBestand} ${artikel.einheit}` },
                      { label: "Lager-Status", value: <LagerBadge status={status} /> },
                      { label: "Mindestbestand", value: `${artikel.mindestbestand} ${artikel.einheit}` },
              ...
              {/* Weitere Infos */}
              {(artikel.liefergroesse || artikel.beschreibung) && (
                <div className="mb-4">
                  <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Weitere Informationen</h3>
                  <div className="border-t border-gray-100">
                    {artikel.liefergroesse && (
                      <div className="py-2.5 flex gap-3 border-b border-gray-100">
      ...
      {tab === "preishistorie" && (
        <div className="space-y-4">
          {loadingPreis ? (
            <p className="text-gray-400 text-sm">Lade Preishistorie…</p>
          ) : preishistorie.length === 0 ? (
            <p className="text-gray-400 text-sm">Keine Preisänderungen vorhanden.</p>
          ) : (
            <>
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-4 text-sm text-gray-600">
                  ...
                  {(() => {
                    ...
                    return (
                      <span className={diff > 0 ? "text-red-600 font-medium" : diff < 0 ? "text-green-700 font-medium" : "text-gray-500"}>
                ...
                <div className="flex rounded-lg border border-gray-200 overflow-hidden text-sm">
                  <button onClick={() => setPreisView("grafik")} className={`px-3 py-1.5 font-medium transition-colors ${preisView === "grafik" ? "bg-green-800 text-white" : "bg-white text-gray-600 hover:bg-gray-50"}`}>Grafik</button>
                  <button onClick={() => setPreisView("tabelle")} className={`px-3 py-1.5 font-medium transition-colors ${preisView === "tabelle" ? "bg-green-800 text-white" : "bg-white text-gray-600 hover:bg-gray-50"}`}>Tabelle</button>
                </div>
              </div>
              {preisView === "grafik" && (
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
                  <PreisChart data={preishistorie} fmt={formatEuro} fmtD={formatDatum} />
                  <p className="text-xs text-gray-400 text-center mt-1">Hover über Datenpunkte für Details</p>
                </div>
              )}
              {preisView === "tabelle" && (
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b border-gray-200">
                      <tr>
                        {[{ label: "Datum", cls: "" }, { label: "Alter Preis", cls: "hidden sm:table-cell" }, { label: "Neuer Preis", cls: "" }, { label: "Differenz", cls: "" }].map((h) => (
                          <th key={h.label} className={`px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide ${h.cls}`}>{h.label}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {[...preishistorie].sort((a, b) => new Date(b.geaendertAm).getTime() - new Date(a.geaendertAm).getTime()).map((p) => {
                        ...
                        return (
                          <tr key={p.id} className="hover:bg-gray-50">
                            ...
                            <td className={`px-4 py-3 font-mono font-medium ${diff > 0 ? "text-red-600" : diff < 0 ? "text-green-700" : "text-gray-500"}`}>
