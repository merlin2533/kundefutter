...
function KontraktNeuInner() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!gueltigVon || !gueltigBis) { setError("Gültigkeitszeitraum erforderlich."); return; }
    ...
    setError("");
    try {
      const res = await fetch("/api/kontrakte", {
        method: "POST",
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 sm:py-8">
      <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6">
        ...
        <span>›</span>
        <span className="text-gray-900 font-medium">Neuer Kontrakt</span>
      </nav>

      <h1 className="text-2xl font-bold text-gray-900 mb-6">Neuen Kontrakt anlegen</h1>

      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 space-y-4">
          <h2 className="text-base font-semibold text-gray-900">Kontraktdaten</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Gültig von <span className="text-red-500">*</span>
              </label>
              <input
                type="date"
                value={gueltigVon}
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Vertragsbedingungen, Hinweise…"
            ...
            {positionen.map((pos, i) => (
              <div key={i} className="border border-gray-200 rounded-lg p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">Kontraktmenge <span className="text-red-500">*</span></label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">Preis (optional)</label>
                    <input
                      type="number"
                      step="0.01"
                      min="0"
        ...
        <div className="flex flex-col-reverse sm:flex-row gap-3 sm:justify-end">
          ...
          >
            {saving ? "Speichere…" : "Kontrakt speichern"}
          </button>
        </div>
      </form>
    </div>
...
export default function KontraktNeuPage() {
  return (
