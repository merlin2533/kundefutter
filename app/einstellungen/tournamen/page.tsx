...
function TourNamenEditor() {
  ...
  async function save(list: string[]) {
    ...
    await fetch("/api/einstellungen", {
      ...
      body: JSON.stringify({ key: "system.tournamen", value: JSON.stringify(list) }),
    });
    setSaving(false);
    setSaved(true);
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      <h2 className="text-lg font-semibold mb-1">Tour-Namen</h2>
      <p className="text-sm text-gray-500 mb-3">Gespeicherte Bezeichnungen für Touren.</p>
      ...
      <div className="flex flex-wrap gap-2 mb-3 min-h-[2rem]">
        ...
        {items.length === 0 && <p className="text-sm text-gray-400">Noch keine Tour-Namen gespeichert</p>}
      </div>
...
export default function TourNamenPage() {
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
        <Link href="/einstellungen" className="hover:text-green-700">Einstellungen</Link>
        <span>›</span>
        <span className="text-gray-800 font-medium">Tour-Namen</span>
      </div>

      <h1 className="text-2xl font-bold mb-6">Tour-Namen</h1>
